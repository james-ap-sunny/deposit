#!/bin/bash
# Direct fix for the database connection issue

echo "=== Direct Fix for Banking App Database Connection ==="

# Create a backup of the connection.py file
echo "Creating backup of connection.py..."
docker exec banking-app cp /app/app/database/connection.py /app/app/database/connection.py.bak

# Create the fixed connection.py file
echo "Creating fixed connection.py file..."
cat << 'EOF' > connection.py.fixed
"""
...
def init_databases(app):
    ...
    try:
        # Get database connection strings directly from environment variables
...
docker cp connection.py.fixed banking-app:/app/app/database/connection.py
